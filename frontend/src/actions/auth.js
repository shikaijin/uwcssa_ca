...
export const checkAuthenticated = () => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/token/verify/`, body, config)
...
export const signup = (username, email, password1, password2) => async dispatch => {
    ...
    try {
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/registration/`, body, config);
...
export const verify = (key) => async dispatch => {
    ...
    try {
        await axios.get(`${process.env.REACT_APP_API_URL}/api/auth/registration/account-confirm-email/${key}/`, config);
...
export const reset_password = (email) => async dispatch => {
    ...
    try {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/password/reset/`, body, config);
...
export const reset_password_confirm = (uid, token, new_password1, new_password2) => async dispatch => {
    ...
    try {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/password/reset/confirm/`, body, config);
